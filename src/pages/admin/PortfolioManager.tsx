...
import { Plus, Edit2, Trash2, X, Loader2, FolderPlus } from "lucide-react";
...
const PortfolioManager = () => {
  ...
  const [sectionModal, setSectionModal] = useState<Partial<Section> | null>(null);
  ...
  const handleSaveSection = async () => {
    if (!sectionModal?.name) return;
    setSavingSection(true);
    const slug = sectionModal.slug || toSlug(sectionModal.name);
    if (sectionModal.id) {
      await supabase.from("portfolio_sections").update({ name: sectionModal.name, slug }).eq("id", sectionModal.id);
    } else {
      const maxOrder = sections.reduce((m, s) => Math.max(m, s.display_order), -1);
      await supabase.from("portfolio_sections").insert({ name: sectionModal.name, slug, display_order: maxOrder + 1 });
    }
  ...
  const handleDeleteSection = async (id: string) => {
    if (!confirm("Delete this section? Videos in it will become uncategorized.")) return;
  ...
  const uncategorized = videos.filter((v) => !v.section_id);
    ...
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => setSectionModal({ display_order: 0 })} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-secondary border border-border text-foreground text-sm font-semibold hover:border-primary/50 transition-all">
      ...
      {/* Section Modal */}
      {sectionModal && (
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSectionModal(null)}>
          <div className="glass-card rounded-2xl border border-border w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h2 className="font-heading font-bold text-lg text-foreground">{sectionModal.id ? "Edit Section" : "Create Section"}</h2>
              <button onClick={() => setSectionModal(null)} className="w-8 h-8 rounded-lg bg-secondary border border-border flex items-center justify-center text-muted-foreground hover:text-foreground"><X size={14} /></button>
            </div>
            <input value={sectionModal.name || ""} onChange={(e) => setSectionModal({ ...sectionModal, name: e.target.value, slug: toSlug(e.target.value) })} placeholder="Section name" className="w-full bg-secondary border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:border-primary/50 outline-none" />
            <p className="text-xs text-muted-foreground">Slug: {sectionModal.slug || toSlug(sectionModal.name || "")}</p>
            <button onClick={handleSaveSection} disabled={savingSection} className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:brightness-110 transition-all flex items-center justify-center gap-2">
              {savingSection && <Loader2 size={16} className="animate-spin" />} {savingSection ? "Saving..." : "Save Section"}
            </button>
          </div>
        </div>
      )}
      ...
      {editing && (
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="glass-card rounded-2xl border border-border w-full max-w-lg p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <select value={editing.section_id || ""} onChange={(e) => setEditing({ ...editing, section_id: e.target.value })} className="w-full bg-secondary border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:border-primary/50 outline-none">
      ...
      {/* Sections + Videos */}
      {!loading && grouped.map(({ section, videos: sectionVideos }) => (
        <div key={section.id} className="mb-6">
          <div className="flex items-center gap-3 mb-3">
            <h2 className="font-heading font-bold text-sm text-primary uppercase tracking-widest">{section.name}</h2>
            <span className="text-[10px] text-muted-foreground">({sectionVideos.length})</span>
            <div className="flex gap-1 ml-auto">
              <button onClick={() => setSectionModal(section)} className="w-7 h-7 rounded-lg bg-secondary border border-border flex items-center justify-center text-muted-foreground hover:text-primary transition-colors"><Edit2 size={12} /></button>
              <button onClick={() => handleDeleteSection(section.id)} className="w-7 h-7 rounded-lg bg-secondary border border-border flex items-center justify-center text-muted-foreground hover:text-destructive transition-colors"><Trash2 size={12} /></button>
            </div>
          </div>
          {sectionVideos.length === 0 && <p className="text-xs text-muted-foreground pl-1 mb-2">No videos in this section.</p>}
      ...
      {!loading && uncategorized.length > 0 && (
        <div className="mb-6">
          <h2 className="font-heading font-bold text-sm text-muted-foreground uppercase tracking-widest mb-3">Uncategorized</h2>
          <div className="space-y-2">
            {uncategorized.map((v) => (
              <div key={v.id} className="glass-card rounded-xl p-4 border border-border flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-foreground truncate">{v.title}</p>
                </div>
                <button onClick={() => setEditing(v)} className="w-9 h-9 rounded-lg bg-secondary border border-border flex items-center justify-center text-muted-foreground hover:text-primary transition-colors"><Edit2 size={14} /></button>
      ...
      {!loading && videos.length === 0 && sections.length === 0 && <p className="text-center py-12 text-muted-foreground">No sections or videos yet.</p>}
    </div>
  );
};
