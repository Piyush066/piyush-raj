...
const AdminDashboard = () => {
  const [stats, setStats] = useState({ videos: 0, services: 0, messages: 0, unread: 0, leads: 0 });

  useEffect(() => {
    const fetchStats = async () => {
      const [vids, svcs, msgs, unread, leads] = await Promise.all([
  ...
  const cards = [
    ...
    { label: "AI Leads", value: stats.leads, icon: Bot, color: "from-orange-600/20 to-primary/5" },
  ];
